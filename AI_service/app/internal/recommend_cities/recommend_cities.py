...
def train_model(df):
    ...
    df = pd.read_excel(df).drop('Unnamed: 0', axis=1)  # Пользователи
    ...
    return best_model, df, dataset, user_features, item_features


def recommend(user_id, model, df, dataset, user_features, item_features, n=5):  # Получаем ТОП-5 рекомендаций
